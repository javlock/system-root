...
public class UnitSection extends Section {
	...
	public void appendAfter(SystemdElement element) throws AlreadyExistsException, OperationNotSupportedException {
		throw new OperationNotSupportedException(" еще нет");
		/*
		 * if (!getAfter().contains(element)) { getAfter().add(element); } else { throw
		 * new AlreadyExistsException(String.format("after contains %s", element)); }
		 */
	}
